...
public class CDUCommand extends CommandBase {
	...
	public String getCommandUsage(ICommandSender sender) {
		return "§ecdu makeGlass/dontDrop/showBlockName/lowerLogs/breakLeaves[true/false]\n"
				+ colorProperty(sender, "makeGlass") + ": Turn felled trees in to "
				+ "glass to view what we think a tree should look like\n" + colorProperty(sender, "dontDrop")
				+ ": The blocks dont fall, " + "but are placed in the air where we think they should go (when combined "
				+ "with breakGlass it just turns the tree in to glass)\n" + colorProperty(sender, "showBlockName")
	...
	private void setBool(ICommandSender sender, String[] args) throws CommandException {
		...
		sender.addChatMessage(
				new TextComponentTranslation(colorProperty(sender, action) + (setValue ? " Enabled" : " Disabled")));
	}
	...
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		if (args.length == 1) {
			return getListOfStringsMatchingLastWord(args, 
					colorProperty(sender, "makeGlass"),
					colorProperty(sender, "dontDrop"), 
					colorProperty(sender, "showBlockName"),
					colorProperty(sender, "lowerLogs"),
					colorProperty(sender, "breakLeaves"));
	...
	private String colorProperty(ICommandSender sender, String property) {
		if (property.equals("makeGlass")) {
			return (getPlayerConfig(sender).makeGlass ? "§a" : "") + "makeGlass§r";
		}
		if (property.equals("dontDrop")) {
			return (getPlayerConfig(sender).dontDrop ? "§a" : "") + "dontDrop§r";
		}
		if (property.equals("showBlockName")) {
			return (getPlayerConfig(sender).showBlockName ? "§a" : "") + "showBlockName§r";
		}
		if (property.equals("lowerLogs")) {
			return (getMainConfigValue("lowerLogs") ? "§a" : "") + "lowerLogs§r";
		}
		if (property.equals("breakLeaves")) {
			return (getMainConfigValue("breakLeaves") ? "§a" : "") + "breakLeaves§r";
		}
		return "";
	}
}
